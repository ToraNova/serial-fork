...
class SerialFork:
    ...
    def udp_transmit(src, dest_sock, dest_addr):
    ...
    def __init__(self, phy, uplinkaddr=None, name0 = '/tmp/serfork0', name1 = '/tmp/serfork1',\
            baud=115200, udphostaddr=('0.0.0.0', 21221), timeout = [0,0,0]):
        ...
        try:
            ...
            if uplinkaddr is None:
                ...
                self.configure_forward(SerialFork.serial_forward, (self.pty1, self.phy) )
                # link udp serv rx - tx pty1
                self.configure_forward(SerialFork.udp_receive, (self.us, self.pty1))
            else:
                self.uladdr = uplinkaddr
                # link physical rx - tx pty1
                self.configure_forward(SerialFork.serial_forward, (self.phy, self.pty1) )
                # link pty1     rx - tx udp client
                self.configure_forward(SerialFork.udp_transmit, (self.pty1, self.us, self.uladdr))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-u", "--uplink", help="uplink udp address (for downlink device use)")
    ...
    sf = SerialFork(args.physical, args.uplink, args.pty0name, args.pty1name, args.baudrate, udphostaddr = (args.saddr, args.sport))
